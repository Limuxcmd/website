...
// 2. Create a new project (e.g. "rate-my-professor")
...
// 6. In your terminal run: npm install firebase
// =============================================================================
...
const firebaseConfig = {
  apiKey: "AIzaSyCKzt3ZNVH1tx7E5ilsKbs4DgwiEAUUO2s",
  authDomain: "website-cb62f.firebaseapp.com",
  projectId: "website-cb62f",
  storageBucket: "website-cb62f.firebasestorage.app",
  messagingSenderId: "769876381378",
  appId: "1:769876381378:web:49d48fa14f0e198a5b355b",
  measurementId: "G-RHH96FTNRB"
};
...
const DB = {
  ...
  delete: async (col, id) => {
    ...
    catch (e) { console.error("DB.delete error:", e); return false; }
  },
  // Query documents by field
  query: async (col, field, value) => {
    try {
...
// Session stored in localStorage (just the user id — actual data comes from Firestore)
const SESSION_KEY = "rmp:session_id";
const getSessionId  = () => { try { return localStorage.getItem(SESSION_KEY); } catch { return null; } };
const setSessionId  = (id) => { try { if (id) localStorage.setItem(SESSION_KEY, id); else localStorage.removeItem(SESSION_KEY); } catch {} };
...
// Seed professors (only inserted once on first load)
...
// MODERATION
// =============================================================================
const BANNED_WORDS = ["fuck","shit","ass","bitch","bastard","crap","damn","dick","pussy","cock","cunt","asshole","motherfucker","bullshit","piss","fag","retard","dumbass","jackass","loser","pathetic","worthless"];
...
async function moderateComment(comment, professorName) {
  ...
  if (RAPIDAPI_KEY === "YOUR_RAPIDAPI_KEY") return { verdict:"approved", reason:"AI not configured" };
  try {
    const res = await fetch("https://ai-text-moderation-toxicity-aspects-sentiment-analyzer.p.rapidapi.com/analyze.php", {
      method:"POST",
      headers:{ "Content-Type":"application/json", "x-rapidapi-host":"ai-text-moderation-toxicity-aspects-sentiment-analyzer.p.rapidapi.com", "x-rapidapi-key":RAPIDAPI_KEY },
      body: JSON.stringify({ text:comment, metadata:{ request_id:"mod-"+Date.now() } }),
    });
    const data = await res.json();
    const item = data?.data?.items?.[0];
    if (!item) return { verdict:"approved", reason:"No moderation data." };
    if (item.moderation?.allowed===false || item.moderation?.decision==="block") {
      const v = item.moderation?.violations?.[0];
      return { verdict:"rejected", reason:v?`Violates policy: ${v.label} (${v.severity})`:"Inappropriate content." };
    }
    const score = item.toxicity?.overall || 0;
    if (score > 0.6) return { verdict:"flagged", reason:`High toxicity score (${(score*100).toFixed(0)}%).` };
    if (!item.safe && item.sentiment?.label==="negative") return { verdict:"flagged", reason:"Flagged as unsafe with negative sentiment." };
...
// HELPERS
// =============================================================================
const avgRating = (ratings, field) => !ratings?.length ? 0 : ratings.reduce((s,r)=>s+r[field],0)/ratings.length;
const wouldTakeAgainPct = (ratings) => !ratings?.length ? 0 : Math.round(ratings.filter(r=>r.wouldTakeAgain).length/ratings.length*100);
...
function ImageUpload({ value, onChange, label, shape="square", size=80 }) {
  ...
  return (
    <div>
      ...
      <div onClick={()=>inputRef.current?.click()} onDragOver={e=>{e.preventDefault();setDragging(true);}} onDragLeave={()=>setDragging(false)} onDrop={e=>{e.preventDefault();setDragging(false);handle(e.dataTransfer.files[0]);}}
        ...
        onMouseEnter={e=>e.currentTarget.style.borderColor="#c9a84c"} onMouseLeave={e=>e.currentTarget.style.borderColor=dragging?"#c9a84c":"#3a3530"}>
        {!value && <div style={{textAlign:"center",padding:6}}><svg width="22" height="22" fill="none" stroke="#6b6458" strokeWidth="1.5" viewBox="0 0 24 24"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg><div style={{fontFamily:"'Crimson Pro',serif",color:"#6b6458",fontSize:10,marginTop:3,lineHeight:1.3}}>Click or<br/>drop</div></div>}
      </div>
...
function BannerUpload({ value, onChange }) {
  const inputRef = useRef();
  const handle = async (file) => { if (!file) return; try { onChange(await readFileAsBase64(file)); } catch(e) { alert(e.message); } };
  return (
    <div>
      <label style={{marginBottom:8}}>Banner Image</label>
      <div onClick={()=>inputRef.current?.click()} style={{height:110,borderRadius:10,cursor:"pointer",overflow:"hidden",position:"relative",background:value?`url(${value}) center/cover`:"#1e1c18",border:"2px dashed #3a3530",display:"flex",alignItems:"center",justifyContent:"center",transition:"border-color 0.2s"}} onMouseEnter={e=>e.currentTarget.style.borderColor="#c9a84c"} onMouseLeave={e=>e.currentTarget.style.borderColor="#3a3530"}>
  ...
  return (
    <span style={{display:"inline-flex",gap:2}}>
      {Array.from({length:max}).map((_,i) => (
        <svg key={i} width={size} height={size} viewBox="0 0 24 24" style={{cursor:onChange?"pointer":"default"}} onMouseEnter={()=>onChange&&setHovered(i+1)} onMouseLeave={()=>onChange&&setHovered(0)} onClick={()=>onChange&&onChange(i+1)}>
          <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" fill={(hovered||value)>i?"#f59e0b":"none"} stroke="#f59e0b" strokeWidth="2"/>
        </svg>
      ))}
    </span>
  );
...
useEffect(() => {
    async function init() {
      // If Firebase isn't configured yet, skip DB and use seed data
      if (firebaseConfig.apiKey === "YOUR_API_KEY") {
        setProfessors(SEED_PROFESSORS);
        setReviews(SEED_REVIEWS);
        setLoading(false);
        return;
      }

      // Timeout after 8 seconds — don't hang forever
      ...
      try {
        // Load professors — seed if empty
        let profs = await DB.getAll("professors");
        if (profs.length === 0) {
          for (const p of SEED_PROFESSORS) await DB.set("professors", p.id, p);
          profs = SEED_PROFESSORS;
        }
        setProfessors(profs);

        // Load reviews — seed if empty
        let revs = await DB.getAll("reviews");
        if (revs.length === 0) {
          for (const r of SEED_REVIEWS) await DB.set("reviews", r.id, r);
          revs = SEED_REVIEWS;
        }
        setReviews(revs);

        // Load users
        const usrs = await DB.getAll("users");
        setUsers(usrs);
  ...
  const logout = () => {
    setCurrentUser(null); setSessionId(null); nav("home"); showToast("Logged out.");
  };

  const updateCurrentUser = async (updated) => {
    setCurrentUser(updated);
    setSessionId(updated.id);
    await DB.set("users", updated.id, updated);
  ...
  if (loading) return (
    <div style={{minHeight:"100vh",background:"#0f0e0c",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:16}}>
      <div style={{width:40,height:40,border:"3px solid #c9a84c33",borderTop:"3px solid #c9a84c",borderRadius:"50%",animation:"spin 0.8s linear infinite"}}/>
      <span style={{color:"#c9a84c",fontFamily:"'Playfair Display',serif",fontSize:18}}>Connecting to database...</span>
  ...
  const filtered = approvedProfs.filter(p=>[p.name,p.department,p.university].some(v=>v.toLowerCase().includes(searchQ.toLowerCase())));
  ...
  return (
    <>
      ...
      {!isFirebaseConfigured && (
        <div style={{background:"#78350f",padding:"10px 24px",textAlign:"center",fontFamily:"'Crimson Pro',serif",color:"#fde68a",fontSize:14}}>
          ⚠ Firebase not configured yet — data is not being saved. Fill in firebaseConfig at the top of App.js.
        </div>
      )}
      ...
      {page==="register"  && <RegisterPage users={users} setUsers={setUsers} nav={nav} showToast={showToast} setVerifyModal={setVerifyModal}/>}
...
const verify = async () => {
  console.log("Verify clicked, input:", input, "expected:", modal.code);
  
  if (input.trim() !== modal.code) { 
    ...
    <div className="modal-overlay">
      <div className="modal-box fade-in">
        <div style={{textAlign:"center",marginBottom:24}}>
          ...
          <p style={{fontFamily:"'Crimson Pro',serif",color:"#9b9082",fontSize:15,lineHeight:1.6}}>A 6-digit code was sent to<br/><span style={{color:"#c9a84c",fontWeight:600}}>{modal.email}</span></p>
        </div>
...
function EditProfileModal({ currentUser, updateCurrentUser, onClose, showToast }) {
  const [form, setForm] = useState({ displayName:currentUser.displayName||currentUser.username, bio:currentUser.bio||"", avatar:currentUser.avatar||"", banner:currentUser.banner||"", location:currentUser.location||"", website:currentUser.website||"" });
  const [saving, setSaving] = useState(false);
  const save = async () => {
    setSaving(true);
    await updateCurrentUser({ ...currentUser, ...form });
    setSaving(false); onClose(); showToast("Profile updated!");
  };
  return (
    <div className="modal-overlay">
      <div className="modal-box fade-in" style={{maxWidth:540}}>
        ...
        {[["Display Name","displayName","Your public name"],["Location","location","City, Country"],["Website","website","https://yoursite.com"]].map(([lbl,key,ph])=>(
          <div key={key} style={{marginBottom:16}}><label>{lbl}</label><input className="form-field" placeholder={ph} value={form[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} style={{marginTop:6}}/></div>
        ))}
        <div style={{marginBottom:20}}><label>Bio</label><textarea className="form-field" rows={3} placeholder="Tell us about yourself..." value={form.bio} onChange={e=>setForm(f=>({...f,bio:e.target.value}))} style={{resize:"vertical",marginTop:6}}/></div>
        <div style={{display:"flex",gap:10}}>
          <button className="btn-primary" onClick={save} disabled={saving} style={{flex:1,padding:13,fontSize:16}}>{saving?"Saving...":"Save Changes"}</button>
        ...
        {filtered.length===0 ? (
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:24}}>
            {filtered.map(prof=>{
              ...
              const avg=avgRating(prof.ratings,"overall"); const diff=avgRating(prof.ratings,"difficulty"); const wta=wouldTakeAgainPct(prof.ratings);
              return (
...
function ProfessorPage({ profId, professors, setProfessors, reviews, setReviews, currentUser, nav, showToast, getUserById }) {
  ...
  const handleReview=async()=>{
    ...
    setModerating(false);
    const newReview={id:uid(),professorId:prof.id,userId:currentUser.id,username:currentUser.username,comment:form.comment.trim(),overall:form.overall,difficulty:form.difficulty,wouldTakeAgain:form.wouldTakeAgain,aiVerdict:result.verdict,aiReason:result.reason,status:result.verdict==="approved"?"approved":result.verdict==="flagged"?"flagged":"rejected",createdAt:now()};
    if (result.verdict!=="rejected"){
      await DB.set("reviews",newReview.id,newReview);
      setReviews(prev=>[...prev,newReview]);
    }
    if (result.verdict==="approved"){
      const updatedRatings=[...prof.ratings,{userId:currentUser.id,overall:form.overall,difficulty:form.difficulty,wouldTakeAgain:form.wouldTakeAgain}];
      await DB.update("professors",prof.id,{ratings:updatedRatings});
      setProfessors(prev=>prev.map(p=>p.id===prof.id?{...p,ratings:updatedRatings}:p));
    }
    ...
    <div style={{minHeight:"100vh",background:"#0f0e0c",padding:"40px 24px"}}>
      <div style={{maxWidth:860,margin:"0 auto"}}>
        <button onClick={()=>nav("home")} style={{background:"none",border:"none",color:"#9b9082",fontFamily:"'Crimson Pro',serif",fontSize:15,cursor:"pointer",marginBottom:28,display:"flex",alignItems:"center",gap:6}}>
          <svg width="16" height="16" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><line x1="19" y1="12" x2="5" y2="12"/><polyline points="12 19 5 12 12 5"/></svg>Back to search
        </button>
          ...
          <div style={{background:"#14120e",border:"1px solid #c9a84c33",borderRadius:14,padding:28,marginBottom:28}}>
            ...
            <div style={{marginBottom:20}}>
              ...
              <div style={{display:"flex",gap:12,marginTop:8}}>
                {[true,false].map(val=>(
                  <button key={String(val)} onClick={()=>setForm(f=>({...f,wouldTakeAgain:val}))} style={{padding:"8px 20px",borderRadius:8,fontFamily:"'Crimson Pro',serif",fontSize:15,border:`1px solid ${form.wouldTakeAgain===val?"#c9a84c":"#3a3530"}`,background:form.wouldTakeAgain===val?"#c9a84c22":"#1e1c18",color:form.wouldTakeAgain===val?"#c9a84c":"#9b9082",transition:"all 0.2s"}}>
                    {val?"Yes ✓":"No ✗"}
                  </button>
                ))}
              </div>
            ...
            <div style={{marginBottom:20}}>
              ...
              <textarea className="form-field" rows={4} placeholder="Share your honest experience..." value={form.comment} onChange={e=>setForm(f=>({...f,comment:e.target.value}))} style={{resize:"vertical",marginTop:6}}/>
            </div>
    ...
    <div style={{minHeight:"100vh",background:"#0f0e0c"}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",padding:"0 24px"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginTop:-52,marginBottom:20,position:"relative",zIndex:2}}>
          <img src={profileUser.avatar||`https://ui-avatars.com/api/?name=${encodeURIComponent(profileUser.displayName||profileUser.username)}&background=2a2620&color=c9a84c&size=200`} alt="" style={{width:100,height:100,borderRadius:"50%",objectFit:"cover",border:"4px solid #0f0e0c"}} onError={e=>e.target.src="https://ui-avatars.com/api/?name=U&background=2a2620&color=c9a84c"}/>
          {isOwn&&<button className="btn-ghost" onClick={()=>setEditProfileOpen(true)} style={{marginBottom:4,display:"flex",alignItems:"center",gap:6}}><svg width="14" height="14" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>Edit Profile</button>}
        </div>
...
function LoginPage({ users, setCurrentUser, nav, showToast }) {
  ...
  const handle=()=>{
    ...
    setCurrentUser(user); setSessionId(user.id);
    showToast("Welcome back, "+(user.displayName||user.username)+"!"); nav("home");
  };
  return (
    <AuthLayout title="Welcome Back" subtitle="Sign in to your account">
      ...
      <div style={{marginTop:20,textAlign:"center",fontFamily:"'Crimson Pro',serif",color:"#6b6458",fontSize:15}}>Don't have an account?{" "}<button onClick={()=>nav("register")} style={{background:"none",border:"none",color:"#c9a84c",cursor:"pointer",fontFamily:"inherit",fontSize:"inherit"}}>Sign up</button></div>
    </AuthLayout>
  );
}
...
// REGISTER
// =============================================================================
function RegisterPage({ users, setUsers, nav, showToast, setVerifyModal }) {
  ...
  return (
    <AuthLayout title="Join the Community" subtitle="Create your free account">
      {[["Email Address","email","you@university.edu","email"],["Username","username","your_username","text"],["Password","password","Min. 6 characters","password"],["Confirm Password","confirm","Repeat password","password"]].map(([lbl,key,ph,type])=>(
        <div key={key} style={{marginBottom:16}}><label>{lbl}</label><input className="form-field" type={type} placeholder={ph} value={form[key]} onChange={e=>setForm(f=>({...f,[key]:e.target.value}))} style={{marginTop:6}}/></div>
      ))}
      <div style={{background:"#1e1c18",borderRadius:8,padding:"10px 14px",marginBottom:20,fontFamily:"'Crimson Pro',serif",color:"#6b6458",fontSize:13,display:"flex",gap:8}}>
        <span style={{color:"#c9a84c"}}>✉</span><span>A 6-digit verification code will be sent to your email.</span>
      </div>
      ...
      <div style={{marginTop:20,textAlign:"center",fontFamily:"'Crimson Pro',serif",color:"#6b6458",fontSize:15}}>Already have an account?{" "}<button onClick={()=>nav("login")} style={{background:"none",border:"none",color:"#c9a84c",cursor:"pointer",fontFamily:"inherit",fontSize:"inherit"}}>Log in</button></div>
    </AuthLayout>
  );
}
...
function SubmitPage({professors,setProfessors,currentUser,nav,showToast}){
  ...
  const handle=async()=>{
    ...
    await DB.set("professors",newProf.id,newProf);
...
function AdminPage({professors,setProfessors,reviews,setReviews,users,currentUser,nav,showToast}){
  ...
  const approveProf=async(id)=>{await DB.update("professors",id,{status:"approved"});setProfessors(prev=>prev.map(p=>p.id===id?{...p,status:"approved"}:p));showToast("Professor approved.");};
  const deleteProf=async(id)=>{await DB.delete("professors",id);setProfessors(prev=>prev.filter(p=>p.id!==id));showToast("Professor removed.");};
  const approveFlagged=async(rv)=>{
    await DB.update("reviews",rv.id,{status:"approved"});
    setReviews(prev=>prev.map(r=>r.id===rv.id?{...r,status:"approved"}:r));
    const prof=professors.find(p=>p.id===rv.professorId);
    if (prof){const updRatings=[...prof.ratings,{userId:rv.userId,overall:rv.overall,difficulty:rv.difficulty,wouldTakeAgain:rv.wouldTakeAgain}];await DB.update("professors",prof.id,{ratings:updRatings});setProfessors(prev=>prev.map(p=>p.id===prof.id?{...p,ratings:updRatings}:p));}
    showToast("Review approved and posted.");
  };
  const deleteRev=async(id)=>{await DB.delete("reviews",id);setReviews(prev=>prev.filter(r=>r.id!==id));showToast("Review deleted.");};
      ...
      <div style={{maxWidth:1000,margin:"0 auto"}}>
        <div style={{display:"flex",alignItems:"center",gap:16,marginBottom:24}}>
          ...
          <div>
            ...
            <p style={{fontFamily:"'Crimson Pro',serif",color:"#9b9082",fontSize:14}}>Powered by Firebase Firestore</p>
          </div>
        </div>
        ...
        {tab==="flagged"&&(flaggedRevs.length===0?<Empty msg="No flagged reviews ✦"/>:
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            {flaggedRevs.map(r=>{const prof=professors.find(p=>p.id===r.professorId);return(
              <div key={r.id} style={{background:"#14120e",border:"1px solid #78350f",borderRadius:12,padding:20}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12,flexWrap:"wrap",gap:8}}>
                  <div><div style={{fontFamily:"'Playfair Display',serif",color:"#e8dcc8",fontSize:16,fontWeight:600,marginBottom:2}}>Review by <span style={{color:"#c9a84c"}}>{r.username}</span></div><div style={{fontFamily:"'Crimson Pro',serif",color:"#9b9082",fontSize:14}}>For: {prof?.name||"Unknown"} · {fmtDate(r.createdAt)}</div></div>
                  <span style={{background:"#78350f",color:"#fde68a",borderRadius:20,padding:"3px 10px",fontFamily:"'Crimson Pro',serif",fontSize:12}}>🚩 Flagged</span>
                </div>
                ...
                <div style={{display:"flex",gap:8}}><button className="btn-approve" onClick={()=>approveFlagged(r)}>Approve ✓</button><button className="btn-danger" onClick={()=>deleteRev(r.id)}>Delete ✗</button></div>
              </div>
            );})}
          </div>
        )}
        {tab==="pending_profs"&&(pendingProfs.length===0?<Empty msg="No pending submissions."/>:
          <div style={{display:"flex",flexDirection:"column",gap:14}}>{pendingProfs.map(p=><AdminCard key={p.id} title={p.name} subtitle={`${p.department} · ${p.university}`} meta={`Submitted: ${fmtDate(p.createdAt)}`} photo={p.photo} extra={p.bio} actions={<><button className="btn-approve" onClick={()=>approveProf(p.id)}>Approve ✓</button><button className="btn-danger" onClick={()=>deleteProf(p.id)}>Delete ✗</button></>}/>)}</div>
        )}
        {tab==="all_profs"&&(allProfs.length===0?<Empty msg="No approved professors."/>:
          <div style={{display:"flex",flexDirection:"column",gap:14}}>{allProfs.map(p=><AdminCard key={p.id} title={p.name} subtitle={`${p.department} · ${p.university}`} meta={`${p.ratings.length} rating(s)`} photo={p.photo} actions={<button className="btn-danger" onClick={()=>deleteProf(p.id)}>Remove ✗</button>}/>)}</div>
        )}
        {tab==="all_reviews"&&(allRevs.length===0?<Empty msg="No approved reviews."/>:
          <div style={{display:"flex",flexDirection:"column",gap:14}}>{allRevs.map(r=>{const prof=professors.find(p=>p.id===r.professorId);return<AdminCard key={r.id} title={`Review by ${r.username}`} subtitle={`For: ${prof?.name||"Unknown"}`} meta={`${fmtDate(r.createdAt)} · ${r.overall}/5`} extra={r.comment} actions={<button className="btn-danger" onClick={()=>deleteRev(r.id)}>Remove ✗</button>}/>;})}</div>
        )}
        {tab==="users"&&(
...
function Empty({msg}){
  return<div style={{textAlign:"center",padding:"60px 0",color:"#6b6458",fontFamily:"'Crimson Pro',serif",fontSize:17}}><div style={{fontSize:32,marginBottom:12}}>✓</div>{msg}</div>;
}
